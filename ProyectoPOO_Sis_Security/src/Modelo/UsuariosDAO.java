...
public class UsuariosDAO {
    ...
    public String conectarPass(String usr, int codAula){
        try{
            ...
            ResultSet rest = sent.executeQuery("SELECT contrasenia FROM USUARIOS WHERE usuario='"+usr+"'");
            rest.next();
            return rest.getString(1);
        }catch(SQLException e){
            e.printStackTrace();
            JOptionPane.showMessageDialog(null, "Error al momento de logear","Login Error",2);
            return null;
        }
    }
    ...
    public DefaultTableModel selectAll(String tabla, int countColumns, DefaultTableModel modelo){
        try (Connection con = DriverManager.getConnection(URL, USER, PASS)) {
